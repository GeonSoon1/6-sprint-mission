...
-- 방법 1: date, time을 각각 정렬
SELECT * FROM orders ORDER BY date DESC, time DESC;

-- 방법 2: date와 time을 합쳐서 정렬 (타임스탬프로 변환)
SELECT * FROM orders ORDER BY (date + time) DESC;
...
FROM order_details 
WHERE pizza_id 
IN (SELECT id FROM pizzas WHERE price > 20);

-- 방법 2: EXISTS 사용
SELECT od.* 
FROM order_details od
WHERE EXISTS (
    SELECT 1 FROM pizzas p 
    WHERE p.id = od.pizza_id AND p.price > 20
);
...
FROM orders 
GROUP BY date HAVING COUNT(*) >= 80 ORDER BY order_count DESC;
...
pizza_id,
SUM(quantity * (
    SELECT price FROM pizzas WHERE id = order_details.pizza_id
)) AS total_revenue
FROM order_details
GROUP BY pizza_id;
...
SELECT 
    ...
    p.size, 
    p.price,
    (SELECT SUM(quantity) FROM order_details WHERE pizza_id = p.id) AS total_quantity
FROM pizzas p
WHERE EXISTS (SELECT 1 FROM order_details WHERE pizza_id = p.id)
ORDER BY total_quantity DESC
LIMIT 10;
...
SELECT 
    ...
    (SELECT SUM(od2.quantity * p2.price)
     ...
     JOIN pizzas p2 ON od2.pizza_id = p2.id
     WHERE od2.order_id IN (SELECT id FROM orders WHERE date = o.date)
    ) AS total_amount
FROM orders o
WHERE o.date >= '2025-03-01' AND o.date < '2025-04-01'
GROUP BY o.date
...
-- 방법 2: 서브쿼리 사용
SELECT 
    (SELECT name FROM pizza_types WHERE id = p.type_id) AS pizza_name,
    p.size AS pizza_size,
    p.price AS pizza_price,
    od.quantity,
    od.quantity * p.price AS total_amount
...
SELECT 
    ...
    (SELECT SUM(od2.quantity * p2.price)
     FROM order_details od2
     JOIN pizzas p2 ON od2.pizza_id = p2.id
     WHERE p2.type_id = pt.id
    ) AS total_revenue
FROM pizza_types pt
WHERE EXISTS (
    SELECT 1 FROM pizzas p 
    WHERE p.type_id = pt.id
    AND EXISTS (SELECT 1 FROM order_details od WHERE od.pizza_id = p.id)
)
ORDER BY total_revenue DESC;
