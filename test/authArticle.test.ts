...
describe('인증이 필요한 게시글 API 통합 테스트', () => {
  ...
  beforeEach(async () => {
    ...
    await agent1.post('/auth/login').send({ email: 'test@test.com', password: 'qwer1234' });
    ...
    await agent2.post('/auth/login').send({ email: 'test2@test.com', password: 'qwer1234' });
  });
  ...
  describe('인증을 거친 후 게시글 생성 POST /articles', () => {
    test('신규 게시글 작성 : title, content, image', async () => {
      const articleResponse = await agent1.post('/articles').send({
        title: '신규 게시글',
        content: '첫번째 게시글 입니다',
        image: 'img.jpg',
      ...
      expect(articleResponse.body.title).toBe('신규 게시글');
    });
    ...
    test('신규 게시글 작성 : image null', async () => {
      ...
      expect(articleResponse.status).toBe(201);
      expect(articleResponse.body.title).toBe('신규 게시글 2');
    });
