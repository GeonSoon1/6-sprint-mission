...
SELECT pizza_id, SUM(quantity) AS order_count
FROM order_details
GROUP BY pizza_id 
...
SELECT p.id, SUM(od.quantity * p.price) AS total_revenue
...
/*
    ...
    주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`,
    피자의 크기를 `pizza_size`, 피자 가격을 `pizza_price`, 수량을 `quantity`,
