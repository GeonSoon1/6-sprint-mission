...
SELECT * FROM products
WHERE id IN (
    SELECT product_id FROM user_favorite_products
    WHERE user_id = 1
)
ORDER BY created_at DESC
LIMIT 10 OFFSET 20;
...
VALUES ('Sample Product', 'This is a sample product description.', 1000, 1);
...
SELECT *, (
  ...
  WHERE products.id = user_favorite_products.product_id
) AS favorite_count
FROM products
...
/*
  ...
  - 1번 유저가 2번 상품에 댓글 작성
*/
INSERT INTO comments (writer_id, product_id, content)
VALUES (1, 2, 'This is a sample comment.');
