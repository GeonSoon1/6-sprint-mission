...
select * from orders order by date desc, time desc offset 0 limit 10;
-- 6. `orders` 테이블에서 오프셋 기반 페이지네이션된 목록을 조회합니다. 페이지 크기가 10이고 최신순일 때 5번째 페이지를 조회하세요.
select * from orders order by date desc, time desc offset 40 limit 10;
...
select pizza_id, sum(quantity) order quantity from order_details group by pizza_id;
...
-- 6. `order_details` 테이블에서 피자별 총 수익을 `total_revenue` 라는 이름으로 구하세요. 
--(수익 = `quantity * price`)
select p.id, sum(p.price*o.quantity) total_revenue 
from pizzas p join order_details o on p.id = o.pizza_id 
group by p.id;
-- 7. 날짜별로 피자 주문 건수(`order_count`)와 총 주문 수량(`total_quantity`)을 구하세요.
select o.date date, count(distinct od.order_id) order_count, sum(od.quantity) total_quantity 
