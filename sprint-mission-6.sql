...
JOIN pizzas
  ON pizzas.id = order_details.pizza_id
WHERE orders.date >= DATE '2025-03-01'
  AND orders.date <  DATE '2025-04-01'
GROUP BY orders.date
ORDER BY orders.date;
...
JOIN pizzas
  ON pizzas.id = order_details.pizza_id
JOIN pizza_types
  ON pizza_types.id = pizzas.type_id
WHERE orders.id = 78
ORDER BY pizza_types.name, pizzas.size, pizzas.price;
