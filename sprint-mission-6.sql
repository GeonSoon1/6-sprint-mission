...
select * from order_details where pizza_id in 
(select id as pizza_id from pizzas where price > 20);
...
select pizza_id, price, total_quantity, (price * total_quantity) as total_revenue
from pizzas
join (
...
select date, (max(order_id)-min(order_id)+1) as total_orders, sum(quantity*price) as total_amount
from orders
join order_details
on orders.id = order_details.order_id
left join pizzas
...
/*
    3. `order`의 `id`가 78에 해당하는 주문 내역들을 조회합니다. 
        주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`, 피자의 크기를 `pizza_size`, 피자 가격을 `pizza_price`, 수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
...
on pizzas.id = order_details.pizza_id
join pizza_types
on pizzas.type_id = pizza_types.id
where order_id = 78;
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
select pizzas.type_id, sum(order_details.quantity * pizzas.price) as earning
from order_details
join pizzas
on order_details.pizza_id = pizzas.id
join pizza_types
on pizzas.type_id = pizza_types.id
group by pizzas.type_id
