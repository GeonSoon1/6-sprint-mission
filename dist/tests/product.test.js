...
describe('Product 통합 테스트', () => {
    ...
    describe('GET /products', () => {
        test('상품 목록 조회 성공 (200)', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get('/products');
            expect(res.status).toBe(200);
            expect(Array.isArray(res.body)).toBe(true);
            if (res.body.length > 0) {
                await (0, supertest_1.default)(app_1.default).get('/products?search=상품').expect(200);
                await (0, supertest_1.default)(app_1.default).get('/products?sort=oldest').expect(200);
                await (0, supertest_1.default)(app_1.default).get('/products?page=0&limit=abc').expect(200);
            }
        });
    });
    describe('GET /products/:id', () => {
        test('상품 상제 조회 성공 (200)', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get(`/products/${productId}`);
            expect(res.status).toBe(200);
            expect(res.body.id).toBe(productId);
        });
        test('상품 상세 조회 실패 - 없는 ID (404)', async () => {
            const res = await (0, supertest_1.default)(app_1.default).get('/products/11111111-1111-4111-8111-111111111111');
