...
SELECT pizza_id, COUNT(order_id) AS total_order_count FROM order_details GROUP BY pizza_id;
...
SELECT date, COUNT(*) AS order_count FROM orders GROUP BY date HAVING COUNT(id) >= 80 ORDER BY order_count DESC;
...
SELECT pizza_id, SUM(quantity * price) AS total_revenue FROM order_details JOIN pizzas ON order_details.pizza_id = pizzas.id GROUP BY pizza_id;
-- ROUND 함수를 사용해서 소수점 둘쨰 자리까지 표현하고 싶었으나 nomeric 에 대해서 이해하지 못해서 사용하지 않았습니다. 추후 이해를 위해서 더 공부할 예정 입니다.

-- 7. 날짜별로 피자 주문 건수(`order_count`)와 총 주문 수량(`total_quantity`)을 구하세요.

SELECT orders.date, COUNT(*) AS order_count, SUM(quantity) AS total_quantity FROM order_details JOIN orders ON order_details.order_id = orders.id GROUP BY orders.date;
...
SELECT pizzas.*,SUM(order_details.quantity) AS total_quantity 
FROM pizzas JOIN order_details ON order_details.pizza_id = pizzas.id 
GROUP BY pizzas.id 
...
SELECT orders.date, COUNT(orders.id) AS total_orders, SUM(order_details.quantity * pizzas.price) AS total_amount 
FROM orders 
...
-- COUNT 에서 DISTINCT를 사용한 이유는 JOIN을 하면서 중복된 order_id가 생겨서 이를 방지하기 위해서 사용했습니다.
-- 하지만 제대로 이해하지 못하여 추후에 더 공부할 예정입니다.
