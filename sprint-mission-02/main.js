...
async function testArticleAxios(){
    ...
    const creatArticleAxios = await artaxitest.createArticleAxios(artAxiosApiData);
    ...
    const updateArticleAxios = await artaxitest.patchArticleAxios(artAxiosApiUpData);

    // ====== DELETE 함수 테스트 ====== 
    const artAxiosDelApiId = 5012;
    const deleteArticleAxios = await artaxitest.deleteArticleAxios(artAxiosDelApiId);
}
