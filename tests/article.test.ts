...
describe('Article 통합 테스트', () => {
  ...
  describe('GET /articles (Search & Sort)', () => {
    test('게시글 검색 및 정렬 조회 성공 (200)', async () => {
      ...
      await request(app).post('/articles').set('Cookie', userToken).send({
        title: '파이썬 기초',
        content: '내용',
      });
      ...
      await request(app).get('/articles?page=0&limit=abc').expect(200);
    ...
    test('좋아요 누른 게시글 목록 조회 시 isLiked: true 확인', async () => {
      ...
      const list = Array.isArray(listRes.body)
        ? listRes.body
        : listRes.body.list;
      const target = list.find((a: any) => a.id === newArticleId);
