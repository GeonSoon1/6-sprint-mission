...
import { CreateUserBodyStruct } from '../structs/usersStructs';
...
export async function userInfoPatch(req: Request, res: Response) {
  const user = req.user;
  const { password, ...rest } = req.body;
...
export async function userUploadProducts(req: Request, res: Response) {
  const user = req.user;
